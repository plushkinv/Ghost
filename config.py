...
max_komissia = 1.5 # указывается в $ (пример 1.2 или 0.15) в нее входит плата за подписание транзакции (но не во всех сетях это учитывается верно) Для минта, сюда так же входит цена картинки.
min_balance = 1.60 # указывается в $. Кошелек с таким балансом и ниже, будет пропускать, считаться пустым.
veroyatnost = 100 # вероятность что кошелек отработает указывается в %
skolko_trans = [1,1] # указывайте диапазон сколько делать транзакций на каждом кошельке. Если это модуль минт, то будет делать столько минтов сколько вы укажете
